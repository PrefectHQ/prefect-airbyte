...
async def test_run_airbyte_connection_sync_standalone_success(
    airbyte_server, airbyte_connection, mock_successful_connection_sync_calls
):

    result = await run_airbyte_connection_sync(airbyte_connection=airbyte_connection)

    assert result == expected_airbyte_sync_result


async def test_run_airbyte_connection_sync_standalone_fail(
    airbyte_server, airbyte_connection, mock_failed_connection_sync_calls, caplog
):

    with pytest.raises(AirbyteSyncJobFailed):
        await run_airbyte_connection_sync(airbyte_connection=airbyte_connection)


async def test_run_airbyte_connection_sync_standalone_cancel(
...
async def test_run_airbyte_connection_sync_standalone_status_updates(
...
async def test_run_airbyte_connection_sync_subflow_synchronously(
    airbyte_server, airbyte_connection, mock_successful_connection_sync_calls
):
    @flow
    def airbyte_sync_sync_flow():
        return run_airbyte_connection_sync(airbyte_connection=airbyte_connection)
...
async def test_run_airbyte_connection_sync_subflow_asynchronously(
    airbyte_server, airbyte_connection, mock_successful_connection_sync_calls
):
    @flow
    async def airbyte_sync_sync_flow():
        return await run_airbyte_connection_sync(airbyte_connection=airbyte_connection)
